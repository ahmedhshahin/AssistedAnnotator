...
class ResNet(nn.Module):
    ...
    def forward(self, x, bbox=None):
        ...
        if self.layer5 is not None:
            segmentation_output = self.layer5(x)
        else:
            segmentation_output = x
        return segmentation_output

    def load_pretrained_ms_modified(self, base_network, nInputChannels=3):
        '''
        This function loads the pretrained model and modifies the first conv layer in case of nInputChannels not equal 3.
        Instead of iterating over modules, we need to iterate over state_dict keys
        We will create a new state_dict which will contain the modified layers and then load this state_dict to the model.
        '''
        new_state = OrderedDict()
        model_state = self.state_dict()
        base_network_state = base_network.state_dict()
        flag = 0
        for key, key_ori in zip(model_state.keys(), base_network_state.keys()):
            # we need only to modify the first conv layer (in case of more than 3 channels), and copy the other weights
            if 'conv' in key and 'conv' in key_ori and ('weight' in key or 'bias' in key):
                if not flag and nInputChannels != 3:
                    n, _, h, w = base_network_state[key_ori].shape
                    new_state[key] = torch.zeros((n, nInputChannels, h, w))
                    new_state[key].data[:, :3, :, :] = base_network_state[key_ori].data
                    for i in range(3, nInputChannels):
                        new_state[key].data[:, i, :, :] = base_network_state[key_ori].data[:, -1, :, :]
                    flag = 1
                # here we copy the weights of the other conv layers
                elif model_state[key].data.shape == base_network_state[key_ori].data.shape:
                    new_state[key] = base_network_state[key_ori]
            # similarly, copying the weights of bn layers
            elif 'bn' in key and 'bn' in key_ori and ('weight' in key or 'bias' in key):
                if model_state[key].data.shape != base_network_state[key_ori].data.shape:
                    continue
                else:
                    new_state[key] = base_network_state[key_ori]
            else:
                new_state[key] = model_state[key]
        # for layers that are in the model and not in the base network (layer 5) models, we copy them from the model to avoid errors when
        # loading the new_state dict
        for key in model_state.keys():
            if key in new_state.keys():
                continue
            else:
                new_state[key] = model_state[key]
