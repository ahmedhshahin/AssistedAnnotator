...
class ScaleNRotate(object):
    ...
    """
    def __init__(self, elems = ['image', 'gt'], rots=(-30, 30), scales=(.75, 1.25), semseg=False):
    ...
    def __call__(self, sample):
        def warpAffine(tmp, M, size, flagval):
            return cv2.warpAffine(tmp, M, size, flagval)
        ...
        for elem in self.elems:
            ...
            tmp = warpAffine(tmp, M, (w,h), flagval)
            sample[elem] = tmp
...
class FixedResize(object):
    ...
    """
    def __init__(self, resolutions=None, flagvals=None, is_val = False):
...
class Normalize(object):
    """
    Normalizes the given batch between 0 and 1
    """
            ...
            if img.ndim == 3:
                ...
                for c in range(C):
                    if img[..., c].max() != 0:
                        img[..., c] -= img[..., c].min()
                        img[..., c] /= img[..., c].max()
            elif img.ndim == 2 and img.max() != 0:
                img -= img.min()
                img /= img.max()
        return sample 

class ZeroCenter(object):
    def __init__(self, elem = 'image'):
        self.elem = elem
    ...
    def __call__(self, sample):
        ...
        img[..., 1] -= 112.9986
        img[..., 2] -= 104.1161
        sample[self.elem] = img
        return sample 

class AddConfidenceMap(object):
    def __init__(self, elem = 'image', hm_type = 'l1l2', tau = 1):
        self.elem = elem
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            if 'meta' in elem or 'crop_relax' in elem:
                continue
            elif 'bbox' in elem:
                tmp = sample[elem]
