...
def load_checkpoint(model, optimizer, fname, device):
        ...
        for state in optimizer.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to(device)
        print("Loaded Checkpoint '{}'(epoch {})".format(fname, checkpoint['epoch']))
    else:
...
def sorting_fn(x):
    '''
    This function is used as a key to the sorting function.
    Example:
        - Input "run_10" --> output: 10
    Old implementation: input: run_9, run_10 --> output: run_10, run_9
    Implementation using this function: input: run_9, run_10 --> output: run_9, run_10
    '''
    return int(x[x.rindex("_")+1:])
...
def fixed_resize(sample, resolution, flagval=None):
    ...
    def resize_image(img, res, flagval_1):
        return cv2.resize(img, res, flagval_1)
    if flagval is None:
    ...
    if sample.ndim == 2 or (sample.ndim == 3 and sample.shape[2] == 3):
        sample = resize_image(sample, resolution[::-1], flagval)
        # sample = cv2.resize(sample, resolution[::-1], interpolation=flagval)
        # sample = misc.imresize(sample, resolution[::-1], interp = 'cubic')
    else:
        tmp = sample
