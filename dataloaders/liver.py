...
class LiverSegmentation(data.Dataset):
    ...
    def __init__(self,
        ...
        self.root = root
        _liver_root = os.path.join(self.root, self.BASE_DIR)
        ...
        for splt in self.split:
            with open(os.path.join(os.path.join(_liver_root, splt + '_imgs.txt')), "r") as f:
                img_lines = f.read().splitlines()

            with open(os.path.join(os.path.join(_liver_root, splt + '_msks.txt')), "r") as f:
                msk_lines = f.read().splitlines()

            
            for i in range(len(img_lines)):
                _image = os.path.join(_liver_root, img_lines[i])
                _mask = os.path.join(_liver_root, msk_lines[i])
...
if __name__ == '__main__':
    ...
    dataset = LiverSegmentation(split=['train'], transform=composed_transforms_tr)
    ...
    for i, sample in enumerate(dataloader):
        ...
        plt.show()
        # concatenated = sample['concat'].numpy()[0]
        # im = concatenated[:3]
        # h_map = concatenated[3]
        # gt = np.squeeze(sample['crop_gt'].numpy())
